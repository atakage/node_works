...
class BBsWriter extends Component {
  state = {
    bbsDate: "",
    bbsAuth: "",
    bbsTitle: "",
  ...
  /*
    react에서는 input 박스에 state 변수를  value값으로 세팅하면 데이터를 외부에서 불러오거나 했을 때 자동으로 해당 input 박스에 값이 세팅되도록 하기 위한 조치
    이 기능을 사용하게 되면 키보드로 문자열을 입력했을 때 문자열이 입력이 안 됨

    각각의 입력박스에 onChange 이벤트를 설정해서 키보드로 입력한 문자열을 state 변수에 세팅해주는 절차가 필요
    문제는 input box가 1,2 개 정도이면 각각의 input box에 event 핸들러를 부착하여 작동시키면 되는데 
    input box가 많아지면 관리가 힘들고 input box의 추가나 변경이 발생하면 유지보수가 매우 어려워짐

    그래서 여러개의 input box가 있을 경우는 한 개의 event 핸들러를 작성하고 공통으로 활용하는 방법이 주로 사용됨
  */
