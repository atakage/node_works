// web에서 request한 path 관련 정보를 추출하기 위한 도구(미들웨어)를 선언
...
router.post("/insert", function(req,res){
    ...
    /*
        ...
        req.query.변수 형식으로 받음

        path variable(path/:변수1/:변수2) 형식으로 받는 데이터는
        req.params.변수1 형식으로 받음

        form에 담겨서 전달되는 데이터는 req.body에 담겨 vo형식으로 받음
    */

    // req.body에 담긴 데이터를 newVO 변수에 복사하고 save() 메서드를 실행한 후 정상적으로 save()되면 callback 함수를 실행하여
    // 현재 DB에 저장된 레코드르 web에 확인
