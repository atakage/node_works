...
class BBsMain extends Component {
  ...
  componentDidMount() {
    ...
    // 이전에 callback 함수가 종료되고 타이머 시간이 경과하면 다시 반복하여 실행
    this.timer = setInterval(() => this.fetchBBsList(), 5000);
  }

  // 혹시 화면에 그려지는 컴포넌트들이 연결이 깨지면 실행되는 method인데
  // timer가 계속 작동되면 react 실행이 deadlock에 빠질 수 있기 때문에 반드시 WillUnmount Method에서 timer를 멈추어 주어야함
  ...
  */
  fetchBBsList = () => {
    ...
    // 새로운 method를 사용하여 데이터를 조회해 오기

    fetch(BBS_MAIN_URL)
      .then(response => {
        // (문자열형으로 리턴된) 가져온 데이터를 json type으로 변환하여 return
        // 다음의 then result 변수에 주입이 됨
  ...
  render() {
    ...
    필드 영역에 선언된 this.state를 분해하여 그중에 bbsList를 별도로 추출해 두면 tag코드에서 사용할 때 {bbsList} 형식으로 사용 가능
